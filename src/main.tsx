...
function App(p:{viewer:Viewer}) {
  ...
  const canonicalChangeSetsQuery=createQuery(()=>({queryKey:['canonicalChangeSets',currentRepo().id],queryFn:()=>orkiaApi.repositoryCanonicalChangeSets(currentRepo().id),enabled:currentTab().view==='project-operations'&&liveRepos().length>0,retry:false}))
